...
int16_t rwtext[1000];  //Read/Write buf
	...
	switch(Appli_state)
	{
		...
		case APPLICATION_READY:
			...
			if(HAL_GPIO_ReadPin(Button_GPIO_Port, Button_Pin) == GPIO_PIN_SET && !process)
			{
					...
					{
						f_read(&myFile, (uint8_t*)&rwtext[0], 1000, &bytesread);
			...
			if (half_to_fill == 1)
			{
				if (size_of_file - f_tell(&myFile) > 1000)
				{
					f_read(&myFile, (uint8_t*)&rwtext[0], 1000, &bytesread);
					half_to_fill = 0;
				}
			...
			else if (half_to_fill == 2)
				{
					if (size_of_file - f_tell(&myFile) > 1000)
					{
						f_read(&myFile, (uint8_t*)&rwtext[500], 1000, &bytesread);
						half_to_fill = 0;
					}
					else
					{
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = LED1_Pin|LED2_Pin|LED3_Pin|LED4_Pin|CLK_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
